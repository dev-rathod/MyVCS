...
int loadInputCommand(char *cmd, struct input_cmd *inputCmd, struct paint_matrix *matrix) {
	...
	int elementNo = 0;
	while ( token != NULL) {
		filterString(token, filteredToken);
		// validate every token towards it correctness against the syntax and load the
		// input command struct object accordingly.
		// printf("elementNo is :%d: and token is :%s:  filtered token is :%s:\n", elementNo, token, filteredToken);
		switch(elementNo) {
		case 0:
			strncpy(cmdType, filteredToken, sizeof(cmdType));
			...
			if((cmdType[0] == WRITE_CMD) && (cmdType[1] != 0)) {
				print_error(INCORRECT_DRAW_COMMAND);
				return 0;
			}
			if((cmdType[0] == RESIZE_CMD) && (cmdType[1] != 0)) {
				print_error(INCORRECT_RESIZE_COMMAND);
				return 0;
			}
			if((cmdType[0] == ERASE_CMD) && (cmdType[1] != 0)) {
				print_error(INCORRECT_ERASE_COMMAND);
				return 0;
			}
			if((cmdType[0] == DELETE_CMD) && (cmdType[1] != 0)) {
				print_error(INCORRECT_DELETE_COMMAND);
				return 0;
			}
		...
		case 1:
			...
			if((cmdType[0] == WRITE_CMD) || (cmdType[0] == ERASE_CMD)) {
				intValue = convertAsciiToNumber(filteredToken);
			...
			if((cmdType[0] == ADD_CMD) || (cmdType[0] == DELETE_CMD)) {
				if((strncmp(filteredToken, "r",1) == 0) || (strncmp(filteredToken, "c", 1) == 0)) {
					inputCmd->param1 = filteredToken[0];
				} else {
					print_error(INVALID_INPUT_TYPE);
					return 0;
				}
			...
			if((cmdType[0] == SAVE_CMD) || (cmdType[0] == LOAD_CMD)) {
				inputCmd->fileName = (char *)malloc(strlen(filteredToken) * sizeof(char));
				strcpy(inputCmd->fileName, filteredToken);
			}
		...
		default:
			...
			if(cmdType[0] == ERASE_CMD) {
				print_error(INCORRECT_ERASE_COMMAND);
				return 0;
			}
			if(cmdType[0] == DELETE_CMD) {
				print_error(INCORRECT_DELETE_COMMAND);
				return 0;
			}
			print_error(INVALID_INPUT_TYPE);
...
/*This function removes the extra ' ' from the input.
* Arguments:
